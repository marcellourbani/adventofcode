...
part2 :: Input -> Int
part2 i = sum $ mid <$> updated
  where
    updated = iUpd i >>= go2
    pmap = precMap i
    prevs l = M.findWithDefault S.empty l pmap
    go2 l = [l' | let l' = go [] l, l /= l']
    go pre end = case end of
      [] -> pre
      x : xs -> case partition (flip S.member $ prevs x) end of
        ([], _) -> go (pre <> [x]) xs
        (p : _, _) -> go pre (p : filter (/= p) end)
