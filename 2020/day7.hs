...
solve s = (S.size $ containers "shiny gold", allBags "shiny gold" - 1)
  where
    ...
    containers s = go S.empty [s]
      where
        go acc keys
          | null newe = acc
          | otherwise = go (S.union acc newe) (S.toList newe)
          where
            newe = S.difference (S.unions $ (parents !) <$> keys) acc
    allBags b
      | null level = 1
