...
solve s = (first ,second) where
  ...
  validityMatrix = sortBy (\(_,a) (_,b) -> compare ( S.size a ) ( S.size b))
    [ ( fst c , S.fromList vc) | c <- constraints,
      let vc = [ fst col | col<- vtc, and $ flip checkcon (snd c) <$> snd col ] ] where
         vtc = zip [0..] $ columns valitickets
  validColumns = go validityMatrix S.empty where
    go [] _       = []
    go (f:fs) old = (f,id): go fs ( S.union old $ snd f) where
